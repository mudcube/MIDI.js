/*
	----------------------------------------------------------
	MIDI/Synesthesia : 2015-07-12 : https://mudcu.be
	----------------------------------------------------------
...
(function () { 'use strict';
	...
	MIDI.Synesthesia.map = function (type) {
		var res = {};
		...
		if (typeof type === 'object') {
			...
			colors = defs[type] || defs['D. D. Jameson (1844)'];
		}
		...
		for (var note = 0, pcolor; note <= 88; note ++) { // creates mapping for 88 notes
			if (colors.data) {
				...
				if (H === S && S === L) { // note color is unset
					color = blendColor(pcolor, colors[(note + 10) % 12]);
				}
