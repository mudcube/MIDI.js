...
(function(root) { 'use strict';
	...
	*/
	...
	root.loadPlugin = function(params) {
		if (typeof params === 'function') {
			params = {callback: params};
		}

		root.soundfontUrl = params.soundfontUrl || root.soundfontUrl;
		...
		root.audioDetect(function(supports) {
			...
			if (connect[api]) {
				...
				if (params.targetFormat) {
					var audioFormat = params.targetFormat;
	...
	*/
	...
	root.loadResource = function(params) {
		var instruments = params.instruments || params.instrument || 'acoustic_grand_piano';
		///
		...
		for (var i = 0; i < instruments.length; i ++) {
			...
			if (instrument === +instrument) { // is numeric
				instruments[i] = root.GM.byId[instrument];
			}
		}
		///
		params.format = root.__audioFormat;
		params.instruments = instruments;
		///
		connect[root.__api](params);
	};

	var connect = {
		webmidi: function(params) {
			// cant wait for this to be standardized!
			root.WebMIDI.connect(params);
		},
		audiotag: function(params) {
			// works ok, kinda like a drunken tuna fish, across the board
			// http://caniuse.com/audio
			requestQueue(params, 'AudioTag');
		},
		webaudio: function(params) {
			// works awesome! safari, chrome and firefox support
			// http://caniuse.com/web-audio
			requestQueue(params, 'WebAudio');
		}
	};

	var requestQueue = function(params, context) {
		var audioFormat = params.format;
		var instruments = params.instruments;
		var onprogress = params.onprogress;
		var onerror = params.onerror;
		var onload = params.callback;
		///
	...
	*/
	...
	// 		}, delay * 1000)
			...
			if (buffer) {
				...
				if (source) {
					if (source.gainNode) {
						...
						gain.linearRampToValueAtTime(-1.0, delay + 0.4);
					}
					///
					if (streaming) {
						if (delay) {
		...
		midi.connect = function(params) {
			setDefaultPlugin(midi);
			midi.setContext(ctx || new AudioContext(), params.callback);
		};
